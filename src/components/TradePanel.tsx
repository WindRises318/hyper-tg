...
export const TradePanel: React.FC<TradePanelProps> = ({ symbol, price, onTrade }) => {
  ...
  const handleTrade = () => {
    ...
    console.log('TradePanel handleTrade click:', { size, numSize, side, leverage });
    if (isNaN(numSize) || numSize <= 0) {
      console.log('Invalid size, returning');
      alert('Please enter a valid size');
      return;
    }
    onTrade(side, numSize, leverage);
    setSize('');
  };
  ...
  return (
    <div className="bg-hl-surface p-4 rounded-lg border border-hl-border flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        <label className="text-xs text-hl-text-muted uppercase">Size ({symbol})</label>
        <div className="relative">
          ...
          />
          <span className="absolute right-3 top-2 text-hl-text-muted text-sm">{symbol}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 pt-2">
        <div className="flex justify-between text-xs">
          <span className="text-hl-text-muted">Cost</span>
          <span className="text-hl-text">
            {size ? (parseFloat(size) * price / leverage).toFixed(2) : '0.00'} USDC
          </span>
        </div>
        <button
