...
export default function App() {
  const [activeTab, setActiveTab] = useState<
    "trade" | "portfolio" | "history" | "profile"
  >("trade");
  ...
  const [tradeViewTab, setTradeViewTab] = useState<"chart" | "orderbook">(
    "chart",
  );
  const [bottomTradeTab, setBottomTradeTab] = useState<
    "trade" | "positions" | "activity"
  >("trade");
  const [isTrading, setIsTrading] = useState(false);
  const [network, setNetwork] = useState<"mainnet" | "testnet">("testnet");
  const [hlAccount, setHlAccount] = useState<string | null>(null);
  const [closingPositionIds, setClosingPositionIds] = useState<Set<string>>(
    new Set(),
  );
    ...
    const init = async () => {
      ...
      try {
        ...
        const [meta, assetCtxs] =
          await hyperliquidService.getInfo("metaAndAssetCtxs");
        if (!isMounted) return;
        ...
        const candles = await hyperliquidService.fetchCandles(
          selectedCoin,
          selectedInterval,
        );
        if (!isMounted) return;
        ...
        const marketState =
          await hyperliquidService.getMarketState(selectedCoin);
        if (!isMounted) return;
        ...
        if (address) {
          ...
          if (hlState && hlState.assetPositions) {
            const hlPositions: Position[] = hlState.assetPositions.map(
              (p: any) => {
        ...
        setError(
          err.message ||
            "Failed to connect to Hyperliquid. Please check your network.",
        );
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const cleanupL2 = hyperliquidService.subscribeToL2Book(
      selectedCoin,
      (data) => {
        setOrderBook(data);
        if (
  ...
  useEffect(() => {
    setPositions((prev) =>
      prev.map((pos) => {
        const currentPrice =
          allPrices[pos.symbol] ||
          (pos.symbol === market.symbol ? market.price : pos.markPrice);
        const pnl =
          pos.side === "long"
  ...
  const handleTrade = async (
    ...
    if (!hlAccount) {
      toast.error(
        "Please configure your Hyperliquid private key in .env.local to trade.",
      );
      return;
    }
    ...
    if (market.price <= 0) {
      toast.error(
        "Waiting for market price to update. Please try again in a moment.",
      );
      return;
    }
    ...
    try {
      ...
      if (coinSize <= 0) {
        toast.error(
          `Size too small. Minimum size is ${Math.pow(10, -szDecimals)} ${selectedCoin}`,
        );
        return;
      }
  ...
  const handleClosePosition = async (id: string) => {
    ...
    if (!hlAccount) {
      toast.error(
        "Please configure your Hyperliquid private key in .env.local to close positions.",
      );
      return;
    }
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full gap-4 py-20 px-6 text-center">
              ...
              <div className="bg-hl-surface p-3 rounded border border-hl-border max-w-xs w-full overflow-hidden">
                <p className="text-xs font-mono text-hl-red break-words">
                  {error}
                </p>
              </div>
              <p className="text-sm text-hl-text-muted">
                  ...
                  <div className="flex bg-hl-bg/80 backdrop-blur-md border-b border-hl-border p-1">
                    <button
                      ...
                      className={cn(
                        ...
                        tradeViewTab === "chart"
                          ? "bg-hl-surface text-hl-green shadow-sm"
                      ...
                      <div className="flex justify-between items-end mb-6 px-1">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2">
                            <h3 className="text-[10px] font-bold text-hl-text-muted uppercase tracking-widest">
                              Market Depth
                            </h3>
                            <div
                              className={cn(
                        ...
                        <div className="flex flex-col gap-0.5">
                          {orderBook?.levels?.[0]
                            ...
                            .map((level, i) => (
                              <div
                                key={`bid-${i}`}
                                className="flex justify-between relative py-0.5 px-1 group hover:bg-hl-green/5 transition-colors"
                              >
                                <div
                                ...
                                />
                                <span className="text-hl-green font-bold z-10">
                                  {parseFloat(level.px).toFixed(2)}
                                </span>
                                <span className="text-hl-text/80 z-10">
                                  {parseFloat(level.sz).toFixed(3)}
                                </span>
                              </div>
                            ))}
                        </div>
                        ...
                        <div className="flex flex-col gap-0.5">
                          {orderBook?.levels?.[1]
                            ...
                            .map((level, i) => (
                              <div
                                key={`ask-${i}`}
                                className="flex justify-between relative py-0.5 px-1 group hover:bg-hl-red/5 transition-colors"
                              >
                                <div
                                ...
                                />
                                <span className="text-hl-red font-bold z-10">
                                  {parseFloat(level.px).toFixed(2)}
                                </span>
                                <span className="text-hl-text/80 z-10">
                                  {parseFloat(level.sz).toFixed(3)}
                        ...
                        >
                          <TradePanel
                            ...
                            onTrade={(side, size, leverage) => {
                              console.log("App onTrade prop called with:", {
                                side,
                                size,
                                leverage,
                              });
                            ...
                            {history.length === 0 ? (
                              ...
                              <div className="flex flex-col gap-2">
                                {history.slice(0, 5).map((item) => (
                                  <div
                                    key={item.id}
                                    className="bg-hl-bg/50 p-3 rounded-xl border border-hl-border flex justify-between items-center group hover:border-hl-green/30 transition-colors"
                                  >
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-[10px] text-hl-text-muted">
                                          {item.dir} {item.size}
                                        </p>
                                      </div>
                                    </div>
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-hl-surface p-4 rounded-xl border border-hl-border">
                  <p className="text-[10px] text-hl-text-muted uppercase mb-1">
                    Total Equity
                  </p>
          ...
          {activeTab === "history" && (
            ...
            >
              {history.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  <h3 className="text-xs font-bold text-hl-text-muted uppercase tracking-wider px-1">
                    Recent Trades
                  </h3>
                  {history.map((item) => (
                    <div
                      key={item.id}
                      className="bg-hl-surface p-4 rounded-xl border border-hl-border flex flex-col gap-3"
                    >
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-hl-text-muted uppercase">
                              {item.dir} {item.size} {item.symbol}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="bg-hl-surface p-4 rounded-xl border border-hl-border flex justify-between items-center">
                  ...
                  <span className="font-mono text-xs text-hl-text truncate max-w-[150px]">
                    {hlAccount
                      ? `${hlAccount.slice(0, 6)}...${hlAccount.slice(-4)}`
                      : "Not Connected"}
                  </span>
                </div>
                ...
                <div className="bg-hl-surface p-4 rounded-xl border border-hl-border flex justify-between items-center">
                  ...
                  <span className="font-mono font-bold text-hl-text">
                    {profile.balance.toFixed(2)} USDC
                  </span>
                </div>
