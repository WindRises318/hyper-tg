...
export function formatPriceTo5SigFigs(price: number) {
  if (!price) return "0";
...
export function orderWireToAction(orders: any[], grouping: string = 'na', builder?: any): any {
  return {
    type: 'order',
    orders,
    grouping,
    ...(builder !== undefined ? { builder: { b: builder.address.toLowerCase(), f: builder.fee } } : {}),
  };
}
...
export function actionHash(action: any, vaultAddress: string | null, nonce: number): Hex {
